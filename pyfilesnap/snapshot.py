...
class Snapshot:
    ...
    def _load_snapshot_data(self, snapshot_file: str) -> dict:
        logging.debug(f"Loading snapshot data from {snapshot_file}")
        start_time = time.time()
        
        # Clean up the snapshot filename
        snapshot_file = snapshot_file.replace('snapshot_snapshot_', 'snapshot_')
        if snapshot_file.endswith('.json.json'):
            snapshot_file = snapshot_file[:-5] + '.json'
        
        archive_path = os.path.join(self.backup_dir, 'snapshots.tar.gz')
        if os.path.exists(archive_path):
            with tarfile.open(archive_path, 'r:gz') as tar:
                try:
                    f = tar.extractfile(snapshot_file)
                    if f is None:
                        raise ValueError(f"Snapshot file {snapshot_file} not found in archive")
                    snapshot_data = json.loads(f.read().decode())
                except Exception as e:
                    logging.error(f"Error loading snapshot from archive: {e}")
                    raise
        else:
            ...
            try:
                ...
                logging.error(f"Error loading snapshot from file: {e}")
                raise
        
        end_time = time.time()
        logging.debug(f"Loaded snapshot data in {end_time - start_time:.4f} seconds")
